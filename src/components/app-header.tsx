...
import { getEmployee, CURRENT_EMPLOYEE_ID } from "@/lib/mock-data";
  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b bg-background/95 px-4 backdrop-blur">
      <div className="relative hidden max-w-md flex-1 md:block">
        ...
        <Input
          placeholder="Search courses, paths, employees…"
          className="pl-9"
        />
      </div>
          ...
          <DropdownMenuTrigger asChild>
            <button className="flex items-center gap-2 rounded-full pr-2 hover:bg-muted">
              <Avatar className="size-8">
                <AvatarFallback className={`${me.avatarColor} text-white text-xs`}>
                  {me.name
                    .split(" ")
                    .map((n) => n[0])
                    .join("")}
